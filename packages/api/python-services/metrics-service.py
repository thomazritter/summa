...
Usage:
    pip install rouge-score bert-score transformers torch flask numpy
    python metrics-service.py
...
def compute_p_accuracy():
    ...
    Input:
    {
        "article_id": 1,
        "summaries": [
            { "profile": "generic", "content": "..." },
            { "profile": "junior", "content": "..." },
            { "profile": "pleno", "content": "..." },
            { "profile": "senior", "content": "..." }
        ]
    }

    Method:
    - Compute pairwise ROUGE-L F1 between all summary pairs
    - Lower ROUGE-L between different profiles = higher personalization
    - P-Accuracy = 1 - average_pairwise_similarity
    - Also compute BERTScore pairwise for semantic divergence

    Returns:
    {
        "article_id": 1,
        "p_accuracy": 0.45,  (0 = all identical, 1 = all completely different)
        "pairwise_rouge": [
            { "pair": ["generic", "junior"], "rouge_l_f1": 0.65 },
            ...
        ],
        "pairwise_bert_f1": [
            { "pair": ["generic", "junior"], "bert_f1": 0.82 },
            ...
        ],
        "interpretation": "..."
    }
...
if __name__ == "__main__":
    logging.info("Metrics Service (ROUGE + BERTScore + P-Accuracy)")
    logging.info("Porta: 5052")
    logging.info("Endpoints:")
    logging.info("  POST /quality        — ROUGE + BERTScore (individual)")
    logging.info("  POST /quality/batch  — ROUGE + BERTScore (lote)")
    logging.info("  POST /p-accuracy     — P-Accuracy (personalizacao)")
    logging.info("  GET  /health         — Status")
